...
app.post('/delivery', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('delivery')
    .insert([{
      name, phone, delivery_start: startValue, delivery_arrive: endValue,
      delivery_date: deliveryDate, under: count, over: twocount, price: indown, user_id
    }])
    .select()
    .single();
...
app.post('/storage', async (req, res) => {
  ...
  const { data, error } = await supabase
    .from('storage')
    .insert([{
      name, phone, mail: email, storage_start_date: startDate, storage_end_date: endDate,
      location: selectValue, small: count, medium: twocount, large: threecount, price: indown, reservation_country: "Mobile", user_id
    }])
    .select()
    .single();
...
app.post('/status-logs', async (req, res) => {
  ...
  const logs = (rawLogs || []).map(log => {
    const kstUpdated = new Date(log.updated_at)
